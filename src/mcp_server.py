...
def create_strava_mcp_server() -> FastMCP:
    ...
    async def get_activity_detail(activity_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Could not fetch streams for activity {activity_id}")
    ...
    async def sync_recent_activities(days_back: int = 7, limit: int = 20) -> Dict[str, Any]:
        ...
        try:
            ...
            for activity in activities:
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Could not fetch streams for activity {activity['id']}")

                    # Convert to TCX format for export
                    sport = Sport.from_strava_type(activity["type"])
                    tcx_data = tcx_processor.convert_strava_to_tcx(
                        activity_detail, streams, sport)
            ...
            logger.error(
                f"Strava API error in sync_recent_activities: {str(e)}")
            return {"error": f"Strava API error: {str(e)}"}
        except Exception as e:
    ...
    async def authenticate_with_code(code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Strava API error in authenticate_with_code: {str(e)}")
            return {"error": f"Strava API error: {str(e)}"}
        except Exception as e:
    ...
    async def get_trainingpeaks_athlete() -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"TrainingPeaks API error in get_trainingpeaks_athlete: {str(e)}")
    ...
    async def list_trainingpeaks_workouts(days_back: int = 30, limit: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"TrainingPeaks API error in list_trainingpeaks_workouts: {str(e)}")
            return {"error": f"TrainingPeaks API error: {str(e)}"}
        except Exception as e:
    ...
    async def get_trainingpeaks_workout_detail(workout_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"TrainingPeaks API error in get_trainingpeaks_workout_detail: {str(e)}")
            return {"error": f"TrainingPeaks API error: {str(e)}"}
        except Exception as e:
            logger.error(
                f"Error in get_trainingpeaks_workout_detail: {str(e)}")
            return {"error": str(e)}
    ...
    async def upload_workout_to_trainingpeaks(workout_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"TrainingPeaks API error in upload_workout_to_trainingpeaks: {str(e)}")
            return {"error": f"TrainingPeaks API error: {str(e)}"}
        except Exception as e:
    ...
    async def sync_strava_to_trainingpeaks(activity_id: str, include_tcx: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if include_tcx:
                try:
                    ...
                    if streams:
                        ...
                        sport = Sport.from_strava_type(
                            strava_activity.get("type", "Run"))
                        tcx_content = tcx_processor.convert_strava_to_tcx(
                            strava_activity, streams, sport)
                except Exception as e:
                    logger.warning(
                        f"Could not generate TCX for activity {activity_id}: {str(e)}")
            ...
            logger.error(
                f"Strava API error in sync_strava_to_trainingpeaks: {str(e)}")
            return {"error": f"Strava API error: {str(e)}"}
        except TrainingPeaksAPIError as e:
            logger.error(
                f"TrainingPeaks API error in sync_strava_to_trainingpeaks: {str(e)}")
            return {"error": f"TrainingPeaks API error: {str(e)}"}
        except Exception as e:
    ...
    async def bulk_sync_strava_to_trainingpeaks(days_back: int = 7, limit: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_syncs = len(
                [r for r in sync_results if r["status"] == "success"])
            failed_syncs = len(
                [r for r in sync_results if r["status"] == "error"])
            ...
            logger.error(
                f"Error in bulk_sync_strava_to_trainingpeaks: {str(e)}")
            return {"error": str(e)}
    ...
    async def get_training_plans() -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"TrainingPeaks API error in get_training_plans: {str(e)}")
            return {"error": f"TrainingPeaks API error: {str(e)}"}
        except Exception as e:
    ...
    async def get_planned_workouts(days_ahead: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"TrainingPeaks API error in get_planned_workouts: {str(e)}")
            return {"error": f"TrainingPeaks API error: {str(e)}"}
        except Exception as e:
    ...
    async def get_training_metrics(days_back: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"TrainingPeaks API error in get_training_metrics: {str(e)}")
            return {"error": f"TrainingPeaks API error: {str(e)}"}
        except Exception as e:
        ...
        try:
            ...
            if tool_name == "list_activities":
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not fetch streams for activity {arguments['activity_id']}")
            ...
            # Handle TrainingPeaks tools
                ...
                if arguments.get("include_tcx", True):
                    try:
                        ...
                        logger.warning(
                            f"Could not generate TCX for activity {arguments['activity_id']}: {str(e)}")
