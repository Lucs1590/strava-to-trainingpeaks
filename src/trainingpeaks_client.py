...
class TrainingPeaksClient:
    ...
    async def _make_request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 401:
                raise TrainingPeaksAPIError("Authentication failed. Check your credentials.")
            elif response.status_code == 403:
                raise TrainingPeaksAPIError("Access forbidden. Check your permissions.")
            elif response.status_code == 429:
                raise TrainingPeaksAPIError("Rate limit exceeded. Please try again later.")
    ...
    async def upload_tcx_file(self, tcx_content: str, workout_title: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code >= 400:
                try:
                    error_data = response.json()
                    error_message = error_data.get("message", f"Upload failed with status {response.status_code}")
                except:
        ...
        try:
            ...
            if tcx_content:
                result = await self.upload_tcx_file(
                    tcx_content, 
                    workout_title=strava_activity.get("name", "Imported from Strava")
                )
            else:
                # Convert Strava activity to TrainingPeaks format
                workout_data = self._convert_strava_to_trainingpeaks(strava_activity)
    ...
    def _convert_strava_to_trainingpeaks(self, strava_activity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        workout_data = {
            ...
            "distance": strava_activity.get("distance", 0) / 1000,  # Convert to km
        ...
        if strava_activity.get("kilojoules"):
            workout_data["energy"] = strava_activity["kilojoules"] * 1000  # Convert to joules
        if strava_activity.get("average_speed"):
            workout_data["avgSpeed"] = strava_activity["average_speed"] * 3.6  # Convert to km/h
        if strava_activity.get("max_speed"):
            workout_data["maxSpeed"] = strava_activity["max_speed"] * 3.6  # Convert to km/h
