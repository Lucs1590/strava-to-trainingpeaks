...
class StravaClient:
    ...
    async def get_activities(self, limit: int = 30, after: Optional[datetime] = None, before: Optional[datetime] = None) -> List[Dict[str, Any]]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise StravaAPIError(f"Failed to fetch activities: {response.status_code} - {response.text}")
    ...
    async def get_activity_detail(self, activity_id: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise StravaAPIError(f"Failed to fetch activity {activity_id}: {response.status_code} - {response.text}")
    ...
    async def get_activity_streams(self, activity_id: str, keys: List[str] = None) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise StravaAPIError(f"Failed to fetch streams for activity {activity_id}: {response.status_code} - {response.text}")
    ...
    async def _refresh_access_token(self) -> None:
        ...
        if not self.refresh_token or not self.client_id or not self.client_secret:
            raise StravaAPIError("Missing refresh token or client credentials for token refresh")
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise StravaAPIError(f"Failed to refresh token: {response.status_code} - {response.text}")
    ...
    async def exchange_code_for_token(self, code: str, redirect_uri: str = "http://localhost:8000/auth") -> Dict[str, str]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise StravaAPIError(f"Failed to exchange code for token: {response.status_code} - {response.text}")
